...
public class AuthControllerYAMLTest extends AbstractIntegrationTest {
	...
	public void testRefresh() throws JsonMappingException, JsonProcessingException {
		...
		var newtokenVO = given().basePath("/auth/refresh").port(TestConfigs.SERVER_PORT)
				.contentType(TestConfigs.CONTENT_TYPE_YML).pathParam("username", tokenVO.getUsername()).header(TestConfigs.HEADER_PARAM_AUTHORIZATION, "Bearer " + tokenVO.getRefreshToken()).when().put("{username}").then().statusCode(200).extract()
				.body().as(TokenVO.class);
